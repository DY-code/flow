...
- 添加邮箱提醒功能，针对“任务计划”中未指定完成时间的任务提醒及时制定计划
...
### 结果
### 下一步计划
- 改进：保存当前选中和展示的节点ID，在打开项目时恢复
- 改进：添加在思维流节点旁显示最新修改时间的选项（hover效果），在项目标题旁添加最新修改时间
- 改进：提供其他颜色的背景选择
- 改进：添加一个global项目“今日待办”，和“任务计划”配合。在“任务计划”中制定每个任务的完成日期，并在对应的日期生成“今日待办”文件。为global项目提供便捷的保存和同步功能
- 改进：提供一个单独维护的项目文件夹，用于存放和管理“今日代办”等不绑定特定任务的负责全局任务管理的flow项目，并在界面提供打开这类项目的快捷入口
- 改进：删除节点时使用ctrl键将子节点也一同删除
...
3.3：
- 改进：支持（基于git的）版本控制与备份，推送到github
- 改进：采用快捷键ctrl+shift+s（本地版本保存）

2.13：
- 改进：编辑思维流区的节点名称时，双击进行编辑状态后，单击将光标定位到对应位置（而不是切换回非编辑状态），并设置为编辑完成后按enter键切换为非编辑状态。

2.11：
- 修复：黑暗主题下思维流区和其他文本编辑区颜色不一致
- 添加功能：使用tab+]键缩进父节点，子节点同时跟随缩进
- 改进：预览模式右侧浮动大纲，不占用原始的预览文本位置，而是设置为光标移动到附近时从界面右侧弹出，光标离开后自动收起
- 改进：在“版本历史”中保存当前版本，并将当前版本对应的json文件备份到浏览器下载目录后，将标题旁的状态圆点显示为黄色

2.10：
- 修复：pwa不能独立加载，关闭运行npm run dev所在终端后无法正常加载
